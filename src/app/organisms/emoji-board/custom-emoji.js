...
class ImagePack {
  ...
  constructor(rawPack, room) {
    ...
    if (pack.display_name) {
      ...
      this.displayName = room ? room.name : undefined;
    }
    if (pack.avatar_url) {
      this.avatar = pack.avatar_url;
    } else {
      this.avatar = room ? room.getMxcAvatarUrl() : undefined;
    }
    this.usage = pack.usage ? pack.usage : ['emoticon', 'sticker'];
    this.attribution = pack.attribution;
    this.images = Object.entries(rawPack.images).map((e) => {
      const data = e[1];
      const shortcode = e[0];
      const mxc = data.url;
      const body = data.body ? data.body : shortcode;
      const { info } = data;
      const usage = data.usage ? data.usage : this.usage;
